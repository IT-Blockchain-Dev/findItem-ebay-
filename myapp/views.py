...
def getApiInfo(request):
    with connection.cursor() as cursor:
        ...
        api_info = []
        for result in row:
            api_info.append(dict(zip(row_headers, result)))
    # print(api_info[0]['app_id'], api_info[0]['cert_id'],
    #       api_info[0]['dev_id'], api_info[0]['token'])
    result = api_info[0]
    res = {"app_id": result['app_id'], "cert_id": result['cert_id'],
           "dev_id": result['dev_id'], "token": result['token']}
    return Response(res)
...
def updateApiInfo(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("UPDATE api_info SET app_id =%s, cert_id =%s , dev_id =%s , token =%s ", [
                       app_id, cert_id, dev_id, token])
...
def uploadData(request):
    ...
    token = 'AgAAAA**AQAAAA**aAAAAA**KlviYQ**nY+sHZ2PrBmdj6wVnY+sEZ2PrA2dj6MHlYapDpOEqAydj6x9nY+seQ**d8oGAA**AAMAAA**'
...
def findItemsAdvanced(token, keyword, min_price, max_price, zip_code, review, ranking, condition, category_id, buy_format):
    ...
    try:
        ...
        token1 = token
        api = finding(domain='svcs.ebay.com', appid=result['app_id'],
                      certid=result['cert_id'], devid=result['dev_id'],
                      token=result['token'], config_file=None)
